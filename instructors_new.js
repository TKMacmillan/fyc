...
for (var i = 0; i < len; i++) {
  ...
  if (index !== -1) {
    ...
    states[state].push([hashids.encode(parseInt(instructorID, 10)), instructorName, hashids.encode(parseInt(instructors[i][3], 10))]);
  }
}
...
for (var c = 0; c < allStatesLen; c++ ) {
  var stream = fs.createWriteStream('./data/instructors216/'+ allStates[c].trim() +'.js');
